...
class PositionCombiner(LeafSystem):
    ...
    def connect_components(self, builder, arm_position_commander, base_position_commander): # Add modules as parameters to be connected to the PositionCombiner
        builder.Connect(
            arm_position_commander.GetOutputPort("desired_spot_arm_position"),
            self.GetInputPort("commanded_arm_position"),
        )
        builder.Connect(

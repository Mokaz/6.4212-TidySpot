...
def _ApplyDriverConfigSim(
    ...
    # TIDYSPOT: Bug made elif isinstance fail
    elif "InverseDynamicsDriver" in driver_config.__class__.__name__:
    # elif isinstance(driver_config, InverseDynamicsDriver):
        ...
        if len(model_instances) == 1:
            ...
            # TIDYSPOT: Injected Adder LeafSystem to boost gripper force
            actuation_force_adder = builder.AddSystem(Adder(2, 10))
            actuation_force_adder.set_name(model_instance_name + ".actuation_force_adder")
            builder.Connect(
                controller.get_output_port(),
                actuation_force_adder.get_input_port(0),
            )
            builder.Connect(
                actuation_force_adder.get_output_port(),
        ...
        builder.ExportInput(
            actuation_force_adder.get_input_port(1),
            model_instance_name + ".added_actuation_force",
        )
